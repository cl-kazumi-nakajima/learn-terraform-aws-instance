terraform {
  ...
  # プロバイダはモジュールのユーザによって定義されるべきなので、モジュール自体では定義しない
  # required_providers {
  #   aws = {
...
resource "aws_launch_configuration" "example" {
  ...
  user_data = templatefile("${path.module}/user-data.sh", { # このモジュールへのファイルシステムパスを参照する
